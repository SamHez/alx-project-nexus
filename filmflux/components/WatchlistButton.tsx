...
const WatchlistButton: React.FC<WatchlistButtonProps> = ({ movie, variant = "full" }) => {
    ...
    return (
        <button
            onClick={handleToggle}
            className={`flex items-center gap-3 px-6 py-3 rounded-xl font-bold transition-all active:scale-95 shadow-lg group ${inWatchlist
        ...
        >
            {inWatchlist ? (
                <>
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-red-500 group-hover:scale-110 transition-transform" viewBox="0 0 20 20" fill="currentColor">
                        <path fillRule="evenodd" d="M3 10a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1z" clipRule="evenodd" />
                    </svg>
